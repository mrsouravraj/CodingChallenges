...
class GenericCodeAnalyzer:
    # Language configs; add new languages by adding entries here
    LANGUAGE_CONFIGS = {
        ".py": LanguageConfig(
            name="Python",
            single_comment=["#"],
    ...
    def __init__(self, path, include_hidden=False, allowed_extensions=None):
        ...
        if allowed_extensions is None:
            # Use keys from LANGUAGE_CONFIGS as allowed extensions by default
        ...
        self.results = []
        self.results_total = defaultdict(lambda: defaultdict(lambda: 0))
        self._total_files = self._total_total = self._total_blank = (
            self._total_comments
        ) = self._total_code = 0
                ...
                for line in f:
                    ...
                    if stripped.startswith("```") or stripped.startswith("~~~"):
                        ...
                        if not in_code_block:
                            ...
                            in_code_block = False
                            fence = None
                        # Fence lines count as blank/comment, not code
                        blank += 1
                        continue
    ...
    def scan(self):
        for root, dirs, files in os.walk(self.path):
            dirs[:] = [d for d in dirs if d.lower() not in IGNORE_FILES]
            ...
            for file in files:
                ...
                if not self.include_hidden and self.is_hidden(full_path):
                    continue
                lang_config = self.detect_language(full_path)
                total, blank, comments, code = self.analyze_file(full_path, lang_config)
                rel_path = os.path.relpath(full_path, self.path)
                self.results_total[lang_config.name]["files"] += 1
                self._total_files += 1
                self.results_total[lang_config.name]["total"] += total
                self._total_total += total
                self.results_total[lang_config.name]["blank"] += blank
                self._total_blank += blank
                self.results_total[lang_config.name]["comments"] += comments
                self._total_comments += comments
                self.results_total[lang_config.name]["code"] += code
    ...
    def print_report(self, detailed=False):
        ...
        print("─" * 85)
        # print(self.results_total)
        for language, stats in self.results_total.items():
            files, total, blank, comments, code = stats.values()
            print(
                f"{language:<35} {files:>8} {total:>8} {blank:>9} {comments:>10} {code:>10}"
            )
        print("─" * 85)
        print(
            f"{'Total':<35} {self._total_files:>8} {self._total_total:>8} {self._total_blank:>9} {self._total_comments:>10} {self._total_code:>10}"
        )
        print("─" * 85)
